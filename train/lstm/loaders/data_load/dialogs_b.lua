...
function dialogs:__init(mode,batch_size)
	...
	local data_path = '../../train/lstm/data/dstc2/'
...
function dialogs:get_next_batch(train_test_valid)
	...
	for i = 1,self.batch_size do
		...
		usr_mat:sub(i,i,1,self.NUSacts,1,U[i]:size(2)):copy(U[i]:view(1,self.NUSacts,max_turns))

		sys_mat = torch.zeros(self.batch_size,self.NSSacts,max_turns)
		sys_mat:sub(i,i,1,self.NSSacts,1,S[i]:size(2)):copy(S[i]:view(1,self.NSSacts,max_turns))
...
--if i==1 and self.alldata[tvt][self.dialog_ptr[tvt]] == 'Mar13_S0A1/voip-14cb91bc48-20130327_202138' then
